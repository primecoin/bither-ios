...
- (BOOL)registerTx:(BTTx *)tx withTxNotificationType:(TxNotificationType)txNotificationType confirmed:(BOOL)confirmed {
    if ([[BTTxProvider instance] isTxDoubleSpendWithConfirmedTx:tx]) {//查看数据库是否存在双花
    ...
    BTTx *compressedTx;
    tx = [[BTHDAccountAddressProvider instance] updateOutHDAccountId:tx];
    if (txNotificationType != txSend) {//当交易为发送模式时，压缩交易数据
...
//    }
    ...
    for (BTOut *out in compressedTx.outs) {//取出转出地址
...
//        }
    ...
    BTTx *txInDb = [[BTTxProvider instance] getTxDetailByTxHash:tx.txHash];//根据获取的tx的hash取出本地数据库里对应的交易数据
    if (txInDb != nil) {//取出对应的交易数据
        for (BTOut *out in txInDb.outs) {//验证转出地址和本地数据库取出的交易数据的转出地址是否一致
            NSString *outAddress = out.outAddress;
...
//            }
        ...
        NSArray *inAddresses = [[BTTxProvider instance] getInAddresses:compressedTx];//根据获取的tx的收入地址
...
//            }
        ...
        [needNotifyHDAccountIdHS addObjectsFromArray:[[BTHDAccountAddressProvider instance] getRelatedHDAccountIdListFromAddresses:inAddresses]];//根据获取的tx的收入地址查出hd账户
        isRegister = needNotifyAddressHashSet.count > 0 || needNotifyHDAccountIdHS.count > 0;//能够查到数据库里有对应的收入地址或者hd账户即认为该交易真实存在
    }
    if (needNotifyAddressHashSet.count > 0 || needNotifyHDAccountIdHS.count > 0) {//如果交易存在保存数据库
        [[BTTxProvider instance] add:compressedTx];
        DDLogDebug(@"register tx %@", [NSString hexWithHash:compressedTx.txHash]);
    }
    for (BTAddress *address in [BTAddressManager instance].allAddresses) {//如果交易真实存在更新余额
...
//    }
    for (NSNumber *hdAccountId in needNotifyHDAccountIdHS) {//如果交易真实存在 更新hd账户余额
