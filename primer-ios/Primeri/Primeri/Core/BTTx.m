...
- (BOOL)signWithPrivateKeys:(NSArray *)privateKeys {//用私钥进行签名
    NSMutableArray *addresses = [NSMutableArray arrayWithCapacity:privateKeys.count],//创建地址空数组
    *keys = [NSMutableArray arrayWithCapacity:privateKeys.count];//创建秘钥空数组
    
    for (NSString *pk in privateKeys) {
        BTKey *key = [BTKey keyWithPrivateKey:pk];//创建模型
        
        if (!key) continue;
        
        [keys addObject:key];//添加私钥地址
        [addresses addObject:key.hash160];//添加钱包地址
    }
    
    for (NSUInteger i = 0; i < self.ins.count; i++) {//交易数组
        BTIn *btIn = self.ins[i];//创建模型
        //取出交易信息中xx的地址位在地址数组的位置
...
- (NSData *)toDataWithSubscriptIndex:(NSUInteger)subscriptIndex {//组装tx消息data
